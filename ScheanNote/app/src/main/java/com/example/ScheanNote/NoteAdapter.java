...
public class NoteAdapter extends BaseAdapter implements Filterable {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        mContext.setTheme(R.style.Theme_AppCompat_DayNight_NoActionBar);
        mInflater=LayoutInflater.from(this.mContext); ;
        ViewHolder holder;
        if(convertView==null){
            holder=new ViewHolder();
            convertView=mInflater.inflate(R.layout.note_layout,null);
            holder.content0=convertView.findViewById(R.id.tv_content);
            holder.time0=convertView.findViewById(R.id.tv_time);
            convertView.setTag(holder);
        }else
        {
            holder=(ViewHolder)convertView.getTag();
        }
        holder.content0.setText(noteList.get(position).getContent());
        holder.time0.setText(noteList.get(position).getTime());
